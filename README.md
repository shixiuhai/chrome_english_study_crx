...
1. **智能单词标记**
   ...
   - 300ms延迟确认机制，避免误操作
   - 支持ESC键取消标记
   - 支持Ctrl+A和Ctrl+C快捷键取消标记
   - 异步初始化标记，等待页面完全加载后处理，避免标记闪烁消失
...
7. **GitHub代码编辑页面自动禁用**
   - 开发页面环境检测机制，能够准确识别当前页面是否为GitHub代码编辑页面
   - 实现功能开关逻辑，当检测到GitHub代码编辑页面时，自动禁用选中翻译功能
   - 确保该功能不影响其他页面的选中翻译正常使用
   - 添加必要的日志记录，便于调试和问题追踪
   - 进行兼容性测试，确保在主流浏览器中正常工作

8. **标点符号自动过滤**
   - 实现标点符号识别机制，准确区分独立标点与包含在文本中的标点
   - 添加过滤规则，对检测到的独立标点符号进行拦截处理
   - 确保过滤功能不影响正常单词和包含标点的完整文本内容的添加
   - 支持中英文标点符号过滤，包括句号(。.)、逗号(，,)、问号(？?)、感叹号(！!)、分号(；;)、冒号(：:)、引号(""''``)、括号(()[]{}())、破折号(--)、省略号(...。。。)等
...
- **性能优化**：
  - 异步初始化标记，等待页面完全加载后处理
  - 减少TreeWalker遍历的文本节点数量（2000 → 500）
  - 优化批量处理大小（100 → 50）
  - 使用acceptNode函数在TreeWalker中直接过滤无效节点
  - 减少每个单词的其他出现标记数量（5 → 3）
  - 减少markOtherOccurrences处理的节点数（50 → 30）
  - 页面加载完成500ms后再处理标记，避免DOM不稳定导致的闪烁
